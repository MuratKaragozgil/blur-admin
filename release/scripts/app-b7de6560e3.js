...
(function () {
    ...
    SiparisListCtrl.$inject = ["$scope", "restService", "$state", "siparisList"];
    angular.module('BlurAdmin.pages.siparis')
...
$templateCache.put('app/pages/siparis/siparis-list.html','<div class="row"><div class="col-md-12"><div class="align-right"><button type="button" class="btn btn-primary btn-raised" ng-click="createArticle()">Siparis Olustur</button></div></div></div><div class="row" style="padding-top: 5px"><div class="col-md-12"><div ba-panel="" ba-panel-class="with-scroll"><div class="horizontal-scroll"><table class="table table-hover"><thead><tr class="black-muted-bg"><th>ID</th><th>Siparis Veren</th><th>E-Posta</th><th>Telefon</th><th>Odeme Tipi</th><th>Action</th></tr></thead><tbody><tr ng-repeat="item in siparisTableData" class="no-top-border"><td>{{item.id}}</td><td>{{item.siparis_veren_kisi}}</td><td>{{item.e_posta}}</td><td>{{item.telefon}}</td><td>{{item.odeme_tipi}}</td><td><a href="/print-qr-code.html?c={{item.id}}&id={{item.id}}&telefon={{item.telefon}}&address={{item.adres}}" target="_blank">Print</a></td></tr></tbody></table></div></div></div></div>');
...
//# sourceMappingURL=../maps/scripts/app-b7de6560e3.js.map
